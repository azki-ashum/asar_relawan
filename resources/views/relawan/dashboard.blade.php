...
@section('content')
{{-- Hero sapaan --}}
<div class="card hero-card border-0 mb-4 overflow-hidden" style="background:linear-gradient(120deg,#0f7a46 0%,#16a34a 55%,#22c55e 100%);">
    <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3 text-white" style="padding:1.6rem 1.75rem;">
        <div>
            <div class="opacity-75 small mb-1"><i class="bi bi-calendar3 me-1"></i>{{ now()->translatedFormat('l, d F Y') }}</div>
            <h3 class="mb-1 text-white">Halo, {{ \Illuminate\Support\Str::title(auth()->user()->name ?? 'Pengaju') }} 👋</h3>
            <div class="opacity-75">Ringkasan seluruh pengajuan kebutuhan relawan.</div>
        </div>
        <a href="{{ route('pengajuan.create') }}" class="btn btn-light fw-semibold"><i class="bi bi-plus-lg me-1"></i>Buat Pengajuan</a>
    </div>
</div>
...
@foreach($needAction as $p)
    ...
    <div class="alert alert-warning d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div class="d-flex align-items-start gap-2">
            ...
            @if($p->catatan_revisi)<span class="d-block small mt-1 opacity-75">Catatan: {{ $p->catatan_revisi }}</span>@endif</div>
        </div>
    ...
    <div class="alert alert-info d-flex flex-wrap justify-content-between align-items-center gap-2">
        <div class="d-flex align-items-start gap-2">
            ...
            {{ $p->catatan_revisi ? 'Admin meminta revisi laporan.' : 'Unggah bukti & laporan untuk menyelesaikannya.' }}</div>
        </div>
        <a href="{{ route('pengajuan.show', $p) }}" class="btn btn-sm btn-success">{{ $p->catatan_revisi ? 'Perbaiki Laporan' : 'Kirim Laporan' }}</a>
    </div>
    @endif
@endforeach
        ...
        <div class="row g-3">
            @php
                $tiles = [
                    ...
                    ['label' => 'Diproses',        'value' => $counts['menunggu'],   'icon' => 'bi-hourglass-split', 'color' => 'text-info'],
    ...
    <div class="col-12 col-lg-9">
        <div class="card shadow-sm h-100">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('pengajuan.index') }}" class="btn btn-sm btn-link">Lihat semua <i class="bi bi-arrow-right ms-1"></i></a>
            </div>
                    ...
                    <table class="table table-hover table-stack align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="cell-actions text-end"><a href="{{ route('pengajuan.show', $p) }}" class="btn btn-sm btn-light">Detail</a></td>
                            </tr>
                            @empty
                            <tr class="no-card"><td colspan="6">
                                <div class="empty-state"><i class="bi bi-inbox"></i>Belum ada pengajuan. <a href="{{ route('pengajuan.create') }}">Buat sekarang</a>.</div>
                            </td></tr>
                            @endforelse
                        </tbody>
                    </table>
