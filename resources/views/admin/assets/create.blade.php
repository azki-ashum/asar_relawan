            ...
            <form action="{{ route('admin.assets.store') }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <select id="type-select" name="type_id" class="form-select" required>
                        ...
                        @foreach($types as $t)
                            <option value="{{ $t->id }}" {{ old('type_id') == $t->id ? 'selected' : '' }}>{{ $t->display_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select">
                        @php $s = old('status', 'active'); @endphp
                ...
                <div class="form-check mb-3">
                    <input name="is_admin_only" id="is_admin_only_asset" type="checkbox" value="1" class="form-check-input" {{ old('is_admin_only') ? 'checked' : '' }}>
