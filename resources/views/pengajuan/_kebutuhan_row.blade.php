...
<div class="kebutuhan-item card border mb-2">
    <div class="card-body py-3">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
            <span class="fw-semibold text-success text-truncate"><i class="bi bi-person-lines-fill me-1"></i>Kebutuhan
                Relawan <span class="kebutuhan-no"></span></span>
            <button type="button" class="btn btn-sm btn-outline-danger btn-remove-kebutuhan flex-shrink-0"
                title="Hapus kebutuhan ini"><i class="bi bi-trash"></i></button>
        </div>
        <div class="row g-2">
            <div class="col-12 col-md-6">
                <label class="form-label small mb-1">Jenis Relawan <span class="text-danger">*</span></label>
                <input type="hidden" name="kebutuhan[{{ $i }}][jenis_relawan]" class="jenis-relawan-val"
                    value="{{ $rawJenis }}">
                <div class="d-flex gap-2 jenis-pair {{ $isCustom ? 'wide-other' : '' }}">
                    ...
                    <input type="text"
                        class="form-control form-control-sm input-jenis-custom {{ $isCustom ? '' : 'd-none' }}"
                        placeholder="Sebutkan jenis relawan..." value="{{ $isCustom ? $rawJenis : '' }}">
                </div>
            </div>
            <div class="col-12 col-md-6">
            ...
            <div class="col-12">
                ...
                <textarea name="kebutuhan[{{ $i }}][detail_tugas]" class="form-control form-control-sm" rows="2"
                    placeholder="Uraikan tugas relawan untuk kebutuhan ini">{{ $row['detail_tugas'] ?? '' }}</textarea>
            </div>
            <div class="col-md-3 mb-2 d-none">
                <label class="form-label small mb-1">Nominal Apresiasi (Rp)</label>
                <input type="number" min="0" step="1000" name="kebutuhan[{{ $i }}][nominal_apresiasi]"
                    class="form-control form-control-sm" value="{{ $row['nominal_apresiasi'] ?? '' }}"
